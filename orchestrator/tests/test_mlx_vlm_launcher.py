...
def test_model_requires_relaxed_weight_loading_for_mlx_gemma4(
    ...
    with override_settings(MODELS_DIR=str(tmp_path)):
        ...
        assert requires_relaxed_weight_loading(model_dir) is True
...
def test_model_requires_relaxed_weight_loading_false_without_kv_shared(tmp_path: Path) -> None:
    with override_settings(MODELS_DIR=str(tmp_path)):
        ...
        assert requires_relaxed_weight_loading(model_dir) is False
